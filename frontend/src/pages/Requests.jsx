...
export default function Requests() {
  ...
  // === Конвертер Base64 обратно в File ===
    ...
    <div className="min-h-[100dvh] bg-admin-bg pb-[120px] md:pb-24 font-sans relative animate-fade-in">
      <div className="p-4 sm:p-8 max-w-4xl mx-auto">
        ...
        <div className="flex overflow-x-auto hide-scrollbar gap-2 mb-6 sm:mb-8 pb-2 sm:pb-0 -mx-4 px-4 sm:mx-0 sm:px-0">
          ...
          >
            Уведомления
            {unreadNotificationsCount > 0 && <span className="px-2 py-0.5 rounded-full text-[10px] bg-white/20 text-white">{unreadNotificationsCount}</span>}
          </button>
          <button 
          ...
          >
            ...
            {unreadSharedCount > 0 && <span className="px-2 py-0.5 rounded-full text-[10px] bg-white/20 text-white">{unreadSharedCount}</span>}
          </button>
        </div>
                ...
                notifications.map((notif) => {
                  ...
                  return (
                    <div key={notif.id} className={`relative ${style.bg} border ${style.border} p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all`}>
                      ...
                      {notif.metadata && (
                        <button 
                          onClick={() => handleOpenNotifPreview(notif)}
                          className="w-full sm:w-auto shrink-0 bg-black/40 hover:bg-black/60 text-white px-4 py-2.5 rounded-xl text-sm font-bold transition-all border border-white/5 flex items-center justify-center gap-2 active:scale-95 z-10"
                        >
