...
export default function Onboarding() {
  ...
  // ГЛОБАЛЬНЫЙ СЧЕТЧИК (База + Текущие новые каналы ТГ)
  ...
  const handleAddVkGroup = async () => {
    ...
    if (currentTotalAccounts >= limits) return setError(`Достигнут лимит: ${limits} аккаунтов.`);
    
    setVkLoading(true); setError('');
    const res = await saveVkGroupWithToken(user.id, vkLinkInput, vkTokenInput);
    if (res.success) {
      setVkConnectedGroups(prev => [...prev, { id: Math.random().toString(), name: res.group?.name, avatar: res.group?.avatar }]);
      setVkLinkInput(''); setVkTokenInput(''); setVkStep(1); 
      fetchAccounts(user.id); // Сразу обновляем глобальный счетчик
  ...
  const handleAddTgChannel = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setTgChannels([...tgChannels, { originalInput: tgInput.trim(), chatId: data.chatId || data.username, title: data.title, username: data.username, avatar: data.avatar }]);
        setTgInput('');
  ...
  return (
    <div className="min-h-[100dvh] bg-admin-bg flex flex-col items-center justify-center p-4 relative font-sans">
      <button onClick={handleSkipAll} className="absolute top-6 right-6 text-gray-400 hover:text-white flex items-center gap-2 bg-gray-900/50 border border-gray-800 px-4 py-2.5 rounded-xl z-20">
      ...
      <div className="w-full max-w-xl bg-admin-card border border-gray-800/60 p-6 sm:p-10 rounded-[2rem] shadow-2xl relative z-10">
        ...
        {step === 'welcome' && (
          <div className="text-center space-y-6 animate-in fade-in">
            ...
            <p className="text-gray-400">Давайте подключим ваши первые каналы.</p>
        ...
        {step === 'vk_setup' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-center">
              <h2 className="text-2xl font-bold text-white flex items-center justify-center gap-2"><span className="w-6 h-6 bg-[#0077FF] rounded flex items-center justify-center text-xs text-white">K</span> ВКонтакте</h2>
            ...
            <div className="space-y-4">
              {vkStep === 1 ? (
                ...
                <div className="flex flex-col gap-3 animate-in slide-in-from-right-4">
                  <div className="bg-[#0077FF]/10 border border-[#0077FF]/20 p-4 rounded-xl text-sm text-gray-300">
                    <p className="font-bold text-white mb-2">Инструкция:</p>
                    <ol className="list-decimal list-inside space-y-1">
                      <li>В настройках вашей группы ВК откройте <b>Работа с API</b>.</li>
                      <li>Нажмите <b>Создать ключ</b>.</li>
                      <li>Выберите галочки: <b>Управление, Фотографии, Стена</b>.</li>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleAddVkGroup} disabled={vkLoading || !vkTokenInput} className="flex-1 bg-[#0077FF] text-white py-4 rounded-xl font-bold disabled:opacity-50">{vkLoading ? 'Проверка...' : 'Подключить группу'}</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-gray-800">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-sm font-semibold text-gray-500 uppercase">Всего аккаунтов</h3>
                <span className="text-xs font-mono bg-gray-800 text-gray-300 px-2 py-1 rounded-md">{currentTotalAccounts} / {limits}</span>
              </div>
              {vkConnectedGroups.length > 0 && (
                <div className="space-y-2 max-h-[180px] overflow-y-auto custom-scrollbar pr-1">
                  {vkConnectedGroups.map((group) => (
                    <div key={group.id} className="flex items-center gap-3 bg-gray-900 border border-gray-800 p-3 rounded-xl animate-in fade-in zoom-in-95">
                      {group.avatar ? <img src={group.avatar} alt="avatar" className="w-10 h-10 rounded-full object-cover shrink-0" /> : <div className="w-10 h-10 rounded-full bg-[#0077FF]/20 flex items-center justify-center shrink-0"><span className="text-[#0077FF] font-bold">K</span></div>}
                      <span className="text-white text-sm font-medium truncate flex-1">{group.name}</span>
            ...
            <button onClick={() => { firstChoice === 'vk' ? setStep('offer_second') : handleSkipAll(); }} className="mt-6 w-full bg-white hover:bg-gray-200 text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2">
        ...
        {step === 'tg_setup' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="text-center"><h2 className="text-2xl font-bold text-white flex items-center justify-center gap-2"><Send size={24} className="text-[#0088CC]" /> Telegram</h2><p className="text-gray-400 mt-2">Добавление каналов для постинга</p></div>
            <div className="min-h-[40px]">{error && <div className="text-red-400 bg-red-400/10 p-3 rounded-xl border border-red-400/20 text-sm">{error}</div>}</div>
            ...
            {!hasTgProfile ? (
              ...
              <div className="space-y-4">
                <div className="bg-[#0088CC]/10 border border-[#0088CC]/20 rounded-2xl p-5 text-sm text-gray-300">
                  ...
                  <ol className="list-decimal list-inside space-y-2">
                    ...
                    <li className="flex flex-wrap items-center gap-2 mt-1">
                      <span>Добавьте в админы бота:</span>
                      <button onClick={() => { navigator.clipboard.writeText('@smmbox_auth_bot'); setCopied(true); setTimeout(()=>setCopied(false),2000); }} className="bg-gray-800 border border-gray-700 px-3 py-1.5 rounded-lg font-mono text-white flex items-center gap-1.5 transition-colors">
                ...
                <div className="flex gap-2">
                  <input type="text" value={tgInput} onChange={(e) => setTgInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddTgChannel()} placeholder="t.me/вашканал или @username" className="flex-1 bg-gray-900 border border-gray-700 text-white rounded-xl p-4 outline-none focus:border-[#0088CC]" />
                    ...
                    <div className="space-y-2 max-h-[180px] overflow-y-auto custom-scrollbar pr-1">
                      {tgChannels.map((channel, idx) => (
                        <div key={idx} className="flex items-center justify-between bg-gray-900 border border-gray-800 p-3 rounded-xl animate-in fade-in zoom-in-95">
                          ...
                          <button onClick={() => removeTgChannel(channel)} className="text-gray-500 hover:text-red-400 p-2 rounded-lg hover:bg-gray-800 transition-colors"><Trash2 size={18} /></button>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <button onClick={saveTgAndContinue} disabled={loading} className="mt-6 w-full bg-[#0088CC] hover:bg-[#0077B3] disabled:opacity-50 text-white font-bold py-4 rounded-xl flex justify-center items-center gap-2">
        ...
        {step === 'offer_second' && (
          <div className="text-center space-y-6 animate-in fade-in">
            <div className="w-20 h-20 bg-green-500/10 text-green-500 rounded-full flex items-center justify-center mx-auto"><CheckCircle2 size={40} /></div>
