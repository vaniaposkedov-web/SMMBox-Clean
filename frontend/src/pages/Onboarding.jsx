          ...
          <div className="space-y-5 sm:space-y-6 text-center animate-in fade-in slide-in-from-right-8 duration-500">
            ...
            {!vkConnected && vkGroupsFetched.length === 0 && (
              <div className="bg-gray-900/50 backdrop-blur-sm p-6 sm:p-8 rounded-2xl border border-gray-700 my-4 sm:my-6">
                <button 
                  ...
                  className="bg-[#0077FF] hover:bg-[#0066DD] disabled:opacity-50 text-white font-bold py-3.5 sm:py-3 px-6 text-sm sm:text-base rounded-xl transition-all shadow-lg shadow-[#0077FF]/30 mx-auto w-full sm:w-auto flex items-center justify-center gap-2 min-h-[48px] active:scale-95"
                >
            ...
            {vkGroupsFetched.length > 0 && !vkConnected && (
              <div className="bg-gray-900/50 border border-gray-700 rounded-2xl p-4 my-4 text-left">
                <h3 className="text-white font-medium mb-3 flex justify-between items-center px-1">
                  <span>Доступные сообщества</span>
                  <span className="text-xs sm:text-sm text-gray-400">{selectedVkGroups.length} из {vkGroupsFetched.length}</span>
                </h3>
                ...
                <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                  {vkGroupsFetched.map(group => (
                    <button 
                      ...
                      className={`w-full flex items-center justify-between p-3 sm:p-4 rounded-xl border transition-all min-h-[56px] active:scale-[0.98] ${selectedVkGroups.includes(group.id) ? 'bg-[#0077FF]/10 border-[#0077FF]/50' : 'bg-gray-800 border-gray-700 hover:border-gray-500'}`}
                    >
                ...
                <button 
                  ...
                  disabled={isVkSaving}
                  className="w-full mt-4 bg-green-600 hover:bg-green-500 disabled:opacity-50 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2 min-h-[48px] active:scale-95"
                >
            ...
            {vkConnected && (
              <div className="bg-green-500/10 border border-green-500/20 p-5 sm:p-6 rounded-2xl my-4 sm:my-6 animate-in fade-in zoom-in-95">
                <CheckCircle2 className="mx-auto text-green-500 mb-2 sm:mb-3" size={32} />
            ...
            <button 
              ...
              className="mt-4 sm:mt-6 w-full bg-blue-600 text-white font-bold py-3.5 sm:py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-blue-500 transition-colors shadow-lg shadow-blue-500/20 text-base min-h-[48px] active:scale-95"
            >
